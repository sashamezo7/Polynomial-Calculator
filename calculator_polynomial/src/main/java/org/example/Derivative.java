...
public class Derivative extends Aplication{
    LinkedList<Termen> rezultat = new LinkedList<>();

    public void derivata(ArrayList<Termen>  polynom1){
        for(int i=0;i<polynom1.size();i++){
            Termen termen = new Termen(polynom1.get(i).getGrad()-1,polynom1.get(i).getCoeficient()*polynom1.get(i).getGrad());
            if(termen.getGrad()>0)
                rezultat.add(termen);
            else if(termen.getGrad()==0 && termen.getCoeficient()!=0)
                rezultat.add(termen);
        }
