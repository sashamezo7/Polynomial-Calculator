...
public class Addition extends Aplication {
    ...
    public void adunare(int grad, double coeficient, ArrayList<Termen> polynom, int ok) {
        ...
        for (int i = 0; i < polynom.size(); i++) {
            if (polynom.get(i).grad == grad) {
                okk = 1;
                if(ok==0) {
                    Termen nou2 = new Termen(grad, polynom.get(i).getCoeficient() + coeficient);
                    rezultat.add(nou2);
                }
                break;
            }
    ...
    public void addPolynomials(ArrayList<Termen> polynom1, ArrayList<Termen> polynom2){
        for(Termen termen : polynom1){
            adunare(termen.getGrad(),termen.getCoeficient(),polynom2,0);
        }
        for(Termen termen : polynom2){
            adunare(termen.getGrad(),termen.getCoeficient(),polynom1,1);
        }
        // sortare
